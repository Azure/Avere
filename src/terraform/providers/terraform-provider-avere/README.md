...
* <a name="auto_wan_optimize"></a>[auto_wan_optimize](#auto_wan_optimize) - (Optional) - enables best core filer performance over a WAN.  The default is true, since most applications will be to an on-prem core filer.
...
* <a name="fixed_quota_percent"></a>[ordinal](#fixed_quota_percent) - (Optional) - specifies the percent of the cache to be initially set when created, but will float dynamically depending on cache usage.  A default of 0 means to use the default provided by the cache.  The fixed quota percent are integers of range [0,100], and the sum must not exceed 100.
