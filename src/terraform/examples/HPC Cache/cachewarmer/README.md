...
The CacheWarmer runs as a service on the jumpbox, and watches the azure storage queue for a job entry.  The job file describes the HPC Cache mount addresses, export path, and path to warm.  For example, the following file is an example of a job entry:
