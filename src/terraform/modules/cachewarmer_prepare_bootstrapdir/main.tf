locals {
  ...
  build_cachewarmer_lines = [
    "set -x",
    "${local.proxy_env} curl --retry 5 --retry-delay 5 --output /tmp/cachewarmer_build.sh https://raw.githubusercontent.com/Azure/Avere/main/src/terraform/modules/cachewarmer_prepare_bootstrapdir/cachewarmer_build.sh",
    "chmod +x /tmp/cachewarmer_build.sh",
    "${local.proxy_env} . /tmp/cachewarmer_build.sh",
  ]

  env_vars = "${local.proxy_env} LOCAL_MOUNT_DIR=${local.mount_dir} BOOTSTRAP_MOUNT_ADDRESS=${var.bootstrap_mount_address} BOOTSTRAP_MOUNT_EXPORT=${var.bootstrap_export_path} BOOTSTRAP_SUBDIR=${var.bootstrap_subdir}"

  prepare_cachewarmer_bootstrap_lines = [
    "set -x",
    "${local.proxy_env} curl --retry 5 --retry-delay 5 --output /tmp/cachewarmer_prepare_bootstrap.sh https://raw.githubusercontent.com/Azure/Avere/main/src/terraform/modules/cachewarmer_prepare_bootstrapdir/cachewarmer_prepare_bootstrap.sh",
