...
locals {
  ...
  service_patchtimeout_file_b64 = base64gzip(replace(file("${path.module}/service.py.patchtimeout"),"\r",""))
  # send the script file to custom data, adding env vars
  cloud_init_file = templatefile("${path.module}/cloud-init.tpl", { service_patchtimeout = local.service_patchtimeout_file_b64, msazure_patchidentity = local.msazure_patchidentity_file_b64, ssh_port = var.ssh_port })
