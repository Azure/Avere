...
locals {
  ...
  command = "systemctl try-restart pd-first-boot && mkdir -p /opt && touch /opt/configure-anvily.py && echo ${local.script_file_b64} | base64 -d | gunzip > /opt/configure-anvily.py && python2 /opt/configure-anvily.py ${var.anvil_data_cluster_ip} ${var.web_ui_password} ${var.dsx_count} ${var.nfs_export_path}"
}
