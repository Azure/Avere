...
resource "null_resource" "install_vdbench_bootstrap" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "curl --retry 5 --retry-delay 5 -o ${local.mount_dir}/${local.bootstrap_dir}/vdbench50407.zip '${var.vdbench_url}'",
      # download the vdbench script last
      "curl --retry 5 --retry-delay 5 -o ${local.mount_dir}/${local.bootstrap_dir}/.bootstrap.vdbench.sh ${local.bootstrap_script}",
      "mv ${local.mount_dir}/${local.bootstrap_dir}/.bootstrap.vdbench.sh ${local.mount_dir}/${local.bootstrap_dir}/bootstrap.vdbench.sh",
