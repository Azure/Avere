...
func createCacheWarmerVmssModel(
	...
	subnetId string) (*compute.VirtualMachineScaleSet, error) {
	...
	cacheWarmerCloudInit := InitializeCloutInit("10.0.1.11", // bootstrapAddress string,
	...
	if err != nil {
		return nil, fmt.Errorf("BUG BUG: customData retrieval hits the following error: %v", err)
	}
	log.Info.Printf("customData: '%s'", customData)
	b64CustomData := base64.StdEncoding.EncodeToString([]byte(customData))
	log.Info.Printf("customDataB64: '%s'", b64CustomData)
		...
		VirtualMachineScaleSetProperties: &compute.VirtualMachineScaleSetProperties{
			...
			VirtualMachineProfile: &compute.VirtualMachineScaleSetVMProfile{
				...
				OsProfile: &compute.VirtualMachineScaleSetOSProfile{
					...
					CustomData:         to.StringPtr(b64CustomData),
...
func ToggleVmss(ctx context.Context, azureClients *AzureClients) error {
	...
	if !vmssExists {
		...
		cacheWarmerVmss, err := createCacheWarmerVmssModel(
