...
$imageTemplates = (Get-Content "$templateDirectory/$moduleDirectory/15-Windows.Workstation.Image.Parameters.json" -Raw | ConvertFrom-Json).parameters.imageTemplates.value

if (Confirm-ImageTemplates $resourceGroupName $imageTemplates) {
    $templateFile = "$templateDirectory/$moduleDirectory/15-Windows.Workstation.Image.json"
    $templateParameters = "$templateDirectory/$moduleDirectory/15-Windows.Workstation.Image.Parameters.json"
