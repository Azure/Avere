...
$imageTemplates = (Get-Content "$rootDirectory/$moduleDirectory/15-Linux.Workstation.Image.Parameters.json" -Raw | ConvertFrom-Json).parameters.imageTemplates.value

if (Confirm-ImageTemplates $resourceGroupName $imageTemplates) {
    $templateFile = "$rootDirectory/$moduleDirectory/15-Linux.Workstation.Image.json"
    $templateParameters = "$rootDirectory/$moduleDirectory/15-Linux.Workstation.Image.Parameters.json"
